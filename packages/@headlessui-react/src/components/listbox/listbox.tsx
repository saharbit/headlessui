...
type Actions =
  | { type: ActionTypes.ToggleListbox }
...
const reducers: {
  ...
  [ActionTypes.ToggleListbox]: state => ({
    ...state,
    listboxState: match(state.listboxState, {
  ...
  [ActionTypes.Search]: (state, action) => {
    ...
    const match = state.items.findIndex(
      item =>
        ...
        item.dataRef.current.textValue?.replace(/\s*/g, '').startsWith(searchQuery)
    )
...
>(
  ...
  const handlePointerUp = React.useCallback(() => {
    dispatch({ type: ActionTypes.ToggleListbox })
...
function Item<TTag extends React.ElementType = typeof DEFAULT_ITEM_TAG, TType = string>(
  ...
  const handlePointerUp = React.useCallback(
    ...
    [dispatch, select, disabled, d, state.buttonRef]
  )
